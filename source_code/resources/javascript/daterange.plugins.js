...
function pickerDateRange(contentId, inputId, options) {
    ...
    var defaults = {
        ...
        minValidDate : '1393603200', //最小可用时间，控制日期选择器的可选力度
        ...
        dayRangeMax : 120, // 日期最大范围(以天计算)
        monthRangeMax : 3, // 日期最大范围(以月计算)
    ...
    $("#"+contentId).html(data_range.content.join(""));
    // 初始化日期选择器面板的HTML代码串
    var wrapper = {
        ta:[
            '<div id="' + this.calendarId + '" class="ta_calendar ta_calendar2 cf">',
                ...
                '<div class="ta_calendar_footer cf" '+ (this.mOpts.autoSubmit ? ' style="display:none" ' : '') +'>',
                    ...
                    '</div>',
                    '<div class="frm_btn">',
                        '<input class="btn" type="button" name="' + this.submitBtn + '" id="' + this.submitBtn + '" value="确定" />',
                        '<input class="btn" type="button" id="' + this.closeBtn + '" value="取消"/>',
                    '</div>',
                '</div>',
            '</div>'
        ]
