    ...
    $.fn.createChart = function(options){
       ...
       //默认配置
        var defaults = {
            ...
            chartType: 'area',                      //图表类型
            dataFormat: 6,                          //数据类型，1：整数，2：浮点数  3：百分比，仅在后面追加百分号  4：时间格式 HH:MM:SS 5：百分比，显示时会乘上100  6：不处理，保留原有格式 
...
//        $(this).css('width', options.width);
        ...
        if (cOptions.chart.type != 'pie'){
            ...
            cOptions.tooltip.formatter =  cOptions.tooltip.formatter || function() {
                return '<b>'+ this.point.name +'</b>: '+ Math.round(this.percentage * 100) / 100 +' % ['+this.point.y+']';
            };
        }   
