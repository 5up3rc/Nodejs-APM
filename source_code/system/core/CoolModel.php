...
abstract class CoolModel {
    ...
    public function connect($database = NULL) {
        ...
        // 检查要连接的db是否有配置
        if (! isset ( Cool::$GC['DB'][$this->db] )) {
            throw new CoolException ( CoolException::ERR_SYSTEM, 'No Database inst key in db.conf.php!' );
        }
        // 设置当前连接的db配置
        $this->conf [$this->db] = Cool::$GC['DB'] [$this->db];
        // 通过配制的db连接引擎 自动初始化对应的helper
        if (strtolower ( Cool::$GC['DB'] [$this->db] ['driver'] ) == 'pdo') {
            $helper = 'PDOHelper';
        } else if (strtolower ( Cool::$GC['DB'] [$this->db] ['driver'] ) == 'mysql') {
            $helper = 'MySQLHelper';
        } else if (strtolower ( Cool::$GC['DB'] [$this->db] ['driver'] ) == 'mysqli') {
            $helper = 'MySQLiHelper';
        } else {
        ...
        if (! isset ( self::$_object [$this->db] )) {
            self::$_object [$this->db] = new $helper ( Cool::$GC['DB'] [$this->db] );
        }
    }
